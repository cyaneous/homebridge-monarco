...
export class ProgrammableSwitchAccessory {
    ...
    monarco.on('rx', (data) => {
      ...
      if(tick % 5 === 0) {
        ...
        if (inputState !== previousInputState) {
          ...
          this.service.updateCharacteristic(this.platform.Characteristic.ProgrammableSwitchEvent, this.state.ProgrammableSwitchEvent);
        }
      }
    });
  }
  ...
  async getProgrammableSwitchEvent(): Promise<CharacteristicValue> {
    const programmableSwitchEvent = this.state.ProgrammableSwitchEvent;

    this.platform.log.debug('Get Characteristic ProgrammableSwitchEvent ->', programmableSwitchEvent);
